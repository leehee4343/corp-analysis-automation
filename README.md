...
4. 필드 누락·형식 의심·중복 의심 등 자동으로 확신할 수 없는 값은 **데이터 검증** 메뉴에 모입니다. "직접 수정" 버튼으로 값을 고치면 해당 이슈가 자동으로 해제됩니다.
5. 각 기업 상세 페이지에서 "엑셀 다운로드"로 `outputs/{기업명}_기업종합보고서.xlsx`를 받거나, "원본 PDF 보기"로 업로드했던 원본을 다시 확인할 수 있습니다.
